...
- SIMD/Assembly 토글: SVT-AV1 인코더의 어셈블리 최적화를 위한 CPU명령어를 탐색하고플래그 강제 활성화/비활성화.
- 성능 및 압축률 선택
...
- **변환 로그 저장**: 변환 결과를 타임스탐프 포함 로그 파일로 기록하며, 성공 완료 시 `.log` 파일은 자동 삭제됩니다.
...
8. 빌드 및 배포 (설치 파일 생성)
- 윈도우 프로그램 추가/제거 및 설치된 파일의 실행 파일과 함께 실행된 윈도우에 .ico 파일을 사용한다.
- build.bat 파일을 생성하여 빌드를 자동화하고 빌드된 파일을 통해 inno setup 스크립트를 작성하여 설치파일로 만든다.
- 프로그램 삭제 시에 파일 및 레지스트리에 남은 내용이 없도록 삭제한다.
...
- 수동 검증 절차 (자동 변환 모드)
  - 1) 레지스트리 HKCU\\Software\\AVIFMaster2의 KeepModifiedTime 값을 1로 설정합니다.
  - 2) AVIFMaster2.exe /auto 원본파일경로 로 실행 후 결과 파일 수정일을 확인합니다.
  - 3) KeepModifiedTime 값을 0으로 바꾸고 동일 절차를 반복하여 ON/OFF 차이를 비교합니다.
